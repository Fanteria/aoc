...
impl TaskRun for Day18 {
        ...
        Self: Sized,
    {
        ...
        Parser::iter_array_sep::<usize, 2>(input, ",")
            ...
            .try_for_each(|coord| {
                *grid
                    .get_mut(coord[1], coord[0])
                    .with_context(|| anyhow!("Failed to get point: {:?}", coord))? = Tail::FallByte;
                Ok::<(), anyhow::Error>(())
            })?;
        let start = Point::new(GRID_SIZE, GRID_SIZE, &grid).context("Cannot find start")?;
        ...
        loop {
            ...
            fall_bytes
                ...
                .try_for_each(|coord| {
                    *new_grid
                        .get_mut(coord[1], coord[0])
                        .with_context(|| anyhow!("Failed to get point: {:?}", coord))? =
                        Tail::FallByte;
                    Ok::<(), anyhow::Error>(())
                })?;
            match find_path(new_grid, start) {
